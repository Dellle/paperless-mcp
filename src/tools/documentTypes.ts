...
export function registerDocumentTypeTools(server: McpServer, api: PaperlessAPI) {
  server.tool(
    ...
    async () => {
      ...
      return asTextResult(await api.getDocumentTypes());
    }
  );
  ...
  server.tool(
    ...
    async (args) => {
      if (!api) throw new Error("Please configure API connection first");
      return asTextResult(await api.createDocumentType(args));
    }
  );
          ...
          view: z
            .object({
              users: z
                .array(z.number())
                .optional()
                .describe("User IDs who can see and assign these document types to documents"),
              groups: z
                .array(z.number())
                .optional()
                .describe("Group IDs who can see and assign these document types to documents"),
            })
            .describe(
          ...
          change: z
            .object({
              users: z
                .array(z.number())
                .optional()
                .describe("User IDs who can modify document type details (name, matching rules)"),
              groups: z
                .array(z.number())
                .optional()
