...
export function registerCorrespondentTools(server: McpServer, api: PaperlessAPI) {
  server.tool(
    ...
    async () => {
      ...
      return asTextResult(await api.getCorrespondents());
    }
  );
  ...
  server.tool(
    ...
    async (args) => {
      if (!api) throw new Error("Please configure API connection first");
      return asTextResult(await api.createCorrespondent(args));
    }
  );
          ...
          view: z
            .object({
              users: z
                .array(z.number())
                .optional()
                .describe("User IDs who can see and assign these correspondents to documents"),
              groups: z
                .array(z.number())
                .optional()
                .describe("Group IDs who can see and assign these correspondents to documents"),
            })
          ...
          change: z
            .object({
              users: z
                .array(z.number())
                .optional()
                .describe("User IDs who can modify correspondent details (name, matching rules)"),
              groups: z
                .array(z.number())
                .optional()
