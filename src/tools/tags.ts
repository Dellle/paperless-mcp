...
export function registerTagTools(server: McpServer, api: PaperlessAPI) {
  server.tool(
    ...
    async () => {
      ...
      return asTextResult(await api.getTags());
    }
  );
  ...
  server.tool(
    ...
    async (args) => {
      if (!api) throw new Error("Please configure API connection first");
      return asTextResult(await api.createTag(args));
    }
  );
  ...
  server.tool(
    ...
    async (args) => {
      if (!api) throw new Error("Please configure API connection first");
      return asTextResult(await api.updateTag(args.id, args));
    }
  );
  ...
  server.tool(
    ...
    async (args) => {
      if (!api) throw new Error("Please configure API connection first");
      return asTextResult(await api.deleteTag(args.id));
    }
  );
          ...
          view: z
            .object({
              users: z
                .array(z.number())
                .optional()
                .describe("User IDs who can see and use these tags"),
              groups: z
                .array(z.number())
                .optional()
                .describe("Group IDs who can see and use these tags"),
            })
          ...
          change: z
            .object({
              users: z
                .array(z.number())
                .optional()
                .describe("User IDs who can modify these tags (name, color, matching rules)"),
              groups: z
                .array(z.number())
                .optional()
                .describe("Group IDs who can modify these tags"),
            })
